...
# MobileNetV3 Model with unfrozen last few inverted residual blocks
class DrowsinessDetector(nn.Module):
    def __init__(self, num_classes=config['NUM_CLASSES'], pretrained=True):
        super(DrowsinessDetector, self).__init__()
        
        # Use MobileNetV3 Large for better feature extraction
        ...
        # In MobileNetV3, the features are organized in sequential blocks
        # We'll unfreeze the last 4-5 blocks (approximately last 10-12 layers)
        
        # Get all the feature blocks
        features = list(self.model.features.children())
        
        # Unfreeze the last 5 blocks (approximately last 12 layers)
        # MobileNetV3 has about 16-17 blocks total
        num_blocks_to_unfreeze = 4
        blocks_to_unfreeze = features[-num_blocks_to_unfreeze:]
        
        for block in blocks_to_unfreeze:
            for param in block.parameters():
                param.requires_grad = True
        
        # Also unfreeze the classifier parameters
...
# Alternative approach: Unfreeze specific inverted residual blocks by name
class DrowsinessDetectorAlt(nn.Module):
    def __init__(self, num_classes=config['NUM_CLASSES'], pretrained=True):
        super(DrowsinessDetectorAlt, self).__init__()
        
        # Use MobileNetV3 Large
        self.model = models.mobilenet_v3_large(pretrained=pretrained)
        ...
        # Unfreeze specific blocks by name pattern
        # MobileNetV3 blocks are named like:
        # blocks.0, blocks.1, ..., blocks.15 (for large version)
        blocks_to_unfreeze = [12, 13, 14, 15]  # Last 4 blocks
        ...
        # Unfreeze the last convolutional layers and classifier
        for name, param in self.model.named_parameters():
            if 'conv' in name and any(str(i) in name for i in [12, 13, 14, 15]):
                param.requires_grad = True
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_acc:
            ...
            torch.save({
                ...
                'val_acc': val_acc,
                'config': config
            }, os.path.join(config['SAVE_DIR'], 'mobilenet_best_model.pth'))
...
def main():
    ...
    # Optimizer with weight decay - only for trainable parameters
    ...
    best_model_path = os.path.join(config['SAVE_DIR'], 'mobile_best_model.pth')
    ...
    }, os.path.join(config['SAVE_DIR'], 'mobilenet_final_model.pth'))
...
def evaluate_model(model, test_loader):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(config['LOG_DIR'], 'mobilenet_confusion_matrix.png'))
...
def plot_training_history(history):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(config['LOG_DIR'], 'mobilenet_training_history.png'))
    plt.close()  # Close to avoid display issues
